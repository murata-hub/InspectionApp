...
const InspectionResultEditOrganizer: React.FC<Props> = ({ inspectionResults, onResultChange }) => {
                                ...
                                <div key={subIndex} className="mb-2">
                                    ...
                                    {(sub === "_no_sub" || expandedSub === sub) && (
                                        <div className="bg-gray-100">
                                            {groupedByMain[main][sub].map((result) => {
                                                // ✅ 元の配列 editResults で result.id にマッチするインデックスを特定
                                                const originalIndex = inspectionResults.findIndex((r) => r.id === result.id);
                                                return (
                                                    <InspectionResultRegisterForm
                                                        key={result.id} // ✅ key は result.id にする
                                                        index={originalIndex} // ✅ 元の正しいインデックスを渡す
                                                        result={result}
...
export default InspectionResultEditOrganizer;
